...
# Listen on all interfaces (0.0.0.0) to accept connections from anywhere
HOST = '0.0.0.0'
PORT = 6666
DEBUGDUMP = False

# Try to import user config but override with internet-accessible settings
try:
    from vankusconf import DEBUGDUMP as USER_DEBUGDUMP
    DEBUGDUMP = USER_DEBUGDUMP
except Exception:
    pass
...
def saveblob(fname, blob):
  ...
  # Create bursts directory if it doesn't exist
  os.makedirs("bursts", exist_ok=True)
...
class ThreadedTCPRequestHandler(socketserver.BaseRequestHandler):
  ...
  def handle(self):
    ...
    try:
      ...
      print("Connect from %s:%i"%peer)
    except Exception as e:
      print(f"Connect from unknown client: {e}")
...
class ThreadedTCPServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
  # Allow address reuse to handle "address already in use" after unclean shutdown
  allow_reuse_address = True
  # Increase socket timeout for better handling of slow connections
  timeout = 60

if __name__ == "__main__":
  # bind to socket and start accepting clients
  print("="*60)
  print("Starting paplon server on %s:%d" % (HOST, PORT))
  print("Server is accessible from:")
  print("  - Local: localhost:%d" % PORT)
  print("  - LAN: <your-local-ip>:%d" % PORT)
  print("  - Internet: Configure port forwarding or use openvpn+portmap")
  print("="*60)
